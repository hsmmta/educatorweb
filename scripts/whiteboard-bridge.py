...
Directory layout created under work-dir:
  ...
  npx_shim.cjs                      Windows only: NODE_OPTIONS preload that lets the E
                                    renderer spawn "npx" (a .cmd shim node cannot exec)
...
def run_step(cmd, cwd, label, timeout, check=True, env=None):
...
# The E renderer invokes hyperframes via bare "npx" (spawnSync/execFileSync,
# no shell). On Windows npx is a .cmd shim, and node/libuv only resolve
# .com/.exe when spawning without a shell, so every such call dies with
# "spawnSync npx ENOENT". Preloading this CJS patch into node (NODE_OPTIONS
# --require) rewrites npx invocations to "<node.exe> .../npm/bin/npx-cli.js".
NPX_SHIM_JS = """\
// Auto-generated by whiteboard-bridge.py (Windows only). Rewrites child_process
// invocations of bare "npx" to "<node> <npm>/bin/npx-cli.js" so the whiteboard
// E renderer can spawn hyperframes without a shell.
"use strict";
...
if (fs.existsSync(npxCli)) {
  const isNpx = (f) => typeof f === "string" && /^npx(\\.cmd)?$/i.test(path.basename(f));
  const wrap = (orig) => function (...a) {
    if (isNpx(a[0])) {
      const hasArgs = Array.isArray(a[1]);
      const args = hasArgs ? a[1] : [];
      const rest = a.slice(hasArgs ? 2 : 1);
      return orig.call(this, process.execPath, [npxCli, ...args], ...rest);
    }
    return orig.apply(this, a);
  };
  for (const name of ["spawn", "spawnSync", "execFile", "execFileSync"]) {
    cp[name] = wrap(cp[name]);
  }
...
def windows_node_env(work_dir):
    """Return an env for the E renderer with the npx shim preloaded (Windows only)."""
    if os.name != "nt":
        return None
    shim = work_dir / "npx_shim.cjs"
    try:
        shim.write_text(NPX_SHIM_JS, encoding="utf-8")
    except OSError as e:
        say(f"WARN: could not write npx shim ({e}); E render may fail to spawn npx")
        return None
    env = os.environ.copy()
    opts = env.get("NODE_OPTIONS", "").strip()
    # NODE_OPTIONS' parser eats backslashes inside quotes; require() accepts forward slashes.
    env["NODE_OPTIONS"] = (opts + " " if opts else "") + f'--require "{shim.as_posix()}"'
    return env
