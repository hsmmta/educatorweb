...
async function doImport() {
  importLoading.value = true; importResult.value = ''
  try {
    ...
    const data = res.data?.data
    if (data?.success) {
      importResult.value = `导入成功！新增 ${data.conceptsAdded || data.knowledgePoints || 0} 个知识点，${data.edgesAdded || data.relationships || 0} 条关系`
      importText.value = ''; importFile.value = null; datasetFile.value = null
